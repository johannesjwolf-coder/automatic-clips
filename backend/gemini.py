...
from google.genai import types, _transformers
...
def _schema(model):
    """Natives Gemini-Schema aus dem Pydantic-Modell. extra="forbid" gilt nur für die Prüfung
    der Antwort; das daraus erzeugte additionalProperties kennt die Gemini-API nicht."""
    def strip(node):
        if isinstance(node, dict):
            return {k: strip(v) for k, v in node.items() if k != "additional_properties"}
        if isinstance(node, list):
            return [strip(v) for v in node]
        return node
    return types.Schema.model_validate(strip(_transformers.t_schema(None, model).model_dump(exclude_none=True)))
    ...
    try:
        ...
        response = client.models.generate_content(model=MODEL,
            ...
            config=types.GenerateContentConfig(
                ...
                response_mime_type="application/json", response_schema=_schema(Analysis),
...
Sekunde {max(0.0, duration - 3):.0f} beginnen. Nur Sprechtext, keine Regieanweisungen."""
            response = client.models.generate_content(model=MODEL,
                contents=types.Content(parts=[part, types.Part(text=prompt)]),
                config=types.GenerateContentConfig(
                    response_mime_type="application/json", response_schema=_schema(Narration),
